...
final class CommentManagerTest
{
	...
	void verifyInverseMethods(String input, String testDescription)
	{
		...
		assertThat(startsWithComment || notStartsWithComment).as(testDescription + " - one method must be true")
															 .isTrue();
		assertThat(startsWithComment && notStartsWithComment).as(testDescription + " - both methods cannot be true")
															 .isFalse();
	}
	...
	private static Stream<Arguments> removeBlockCommentsProvider()
	{
		return Stream.of(
				...
				Arguments.of("String s = \"/* This is not a comment */\";",
